...
sub GetDistro() {
	...
	if ( -e "/etc/mandrake-release" ) {
		...
		if ($release =~ /^Darwin\s+(\d+)\.(\d+)/) {
		   ...
		   $distro="$1$2";
		   # further qualify if HP-UX SRP installed and enabled
                   if ( -x '/opt/hpsrp/bin/util/srp_info') {
		     # possible distros: HP-UX11.31SRPhost or HP-UX11.31SRPcont
	             my $srpmode  = `/opt/hpsrp/bin/util/srp_info -m -S cmds`;
	             my $srpscope = `/opt/hpsrp/bin/util/srp_info -s -S cmds`;
...
sub getSupportedOSHash () {
    my %osHash = ("LINUX" => [
			      ...
			      "RHFC5","RHFC6","RHFC7","RHFC8",
                              "RHFC9","RHFC10","RHFC11","RHFC12",
                              "RHFC13","RHFC14","RHFC15","RHFC16",
		  ...
		  "HP-UX" => [
			      ...
			      "HP-UX11.31",
			      "HP-UX11.31SRPhost", "HP-UX11.31SRPcont"
			      ],
...
#    &dumpServiceInfo; # great for debuging service information issues
   ...
   # OS dependent error messages (after configuring file locations)
    ...
    if ( $distro eq "HP-UX11.31SRPcont" ) {
        # reinitialize Bastille var dir on first use of Bastille in SRP container
	#   possible Bastille remnants from SRP host at container creation time
	my $srp_init_flag = "/var/opt/sec_mgmt/bastille/._SRPcont_init";
	unless ( -e $srp_init_flag ) {
          &B_log("ACTION","Initializing Bastille in SRP system container\n");
	  system( "rm -rf /var/opt/sec_mgmt/bastille/*") == 0
	     or &B_log("ERROR", "SRP Cont INIT, Error deleting bastille var directory!");
	  mkdir "/var/opt/sec_mgmt/bastille/baselines", 0555;
	  chown 2,2,"/var/opt/sec_mgmt/bastille/baselines";
	  chmod 0555, "/var/opt/sec_mgmt/bastille/baselines";
	  mkdir "/var/opt/sec_mgmt/bastille/tmp", 0555;
	  chown 2,2,"/var/opt/sec_mgmt/bastille/tmp";
	  chmod 0555,"/var/opt/sec_mgmt/bastille/tmp";
	  system( "touch $srp_init_flag" ) == 0
	     or &B_log("ERROR", "SRP Cont INIT flag file create failed!");
	  system( "rm /etc/opt/sec_mgmt/bastille/HPWeights.txt");
	}
    }
...
sub B_close_plus {
    ...
    # avoid copy and backup if file never changed - minimizes scope of Bastille "-revert" processing
...
sub checkProcsForService ($;$) {
  ...
  foreach my $psname (@psnames) {
    $psname .= '\b' ;  # avoid embedded match; anchor to trailing word boundry
